...
const form = useForm({
    ...
    owner_name: props.workshop.owner_name || '',
    phone: props.workshop.phone,
    email: props.workshop.email || '',
    address: props.workshop.address || '',
    subscription_plan: props.workshop.subscription_plan,
    subscription_expires_at: props.workshop.subscription_expires_at
        ? props.workshop.subscription_expires_at.slice(0, 10)
        : '',
    is_active: props.workshop.is_active,
    director_name: props.workshop.user?.name || '',
    director_phone: props.workshop.user?.phone || '',
    director_password: '',
});
...
<template>
    <Head :title="`${workshop.name} — tahrirlash`" />

    <AuthenticatedLayout>
        <template #header>
            <div class="flex items-center justify-between">
                <h2 class="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
                    {{ workshop.name }} — o'zgartirish
                </h2>
                <Link
                        ...
                        <form @submit.prevent="submit">
                            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                                ...
                                <div class="border-t border-gray-200 pt-4 sm:col-span-2 dark:border-gray-700">
                                    ...
                                    <p class="text-sm text-gray-500 dark:text-gray-400">Parolni faqat o'zgartirmoqchi bo'lsangiz to'ldiring, aks holda bo'sh qoldiring.</p>
                                </div>
                                ...
                                <div class="sm:col-span-2">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Yangi parol</label>
                                    <input
