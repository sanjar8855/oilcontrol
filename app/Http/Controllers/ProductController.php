...
class ProductController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        try {
            ...
            if ($product->stock_quantity > 0 && $product->track_inventory) {
                ...
                // Auto-create expense for product purchase
    ...
    {
        // Faqat o'z ustaxonasining mahsulotlarini ko'rish mumkin
    ...
    {
        // Faqat o'z ustaxonasining mahsulotlarini tahrirlash mumkin
        if ($product->workshop_id !== $request->user()->workshop->id) {
            abort(403);
        }
    ...
    {
        // Faqat o'z ustaxonasining mahsulotlarini yangilash mumkin
        if ($product->workshop_id !== $request->user()->workshop->id) {
            abort(403);
        }
    ...
    {
        // Faqat o'z ustaxonasining mahsulotlarini o'chirish mumkin
        if ($product->workshop_id !== $request->user()->workshop->id) {
            abort(403);
        }
    ...
    public function processStockAdjustment(Request $request, Product $product): RedirectResponse
    {
        ...
        try {
            ...
            // Auto-create expense for product purchase (only for 'in' type)
            if ($validated['type'] === 'in') {
